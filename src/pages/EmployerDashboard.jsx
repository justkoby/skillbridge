...
const EmployerHome = ({ user, onNavigate }) => {
  const greeting = (() => {
  ...
  return (
    <div>
      ...
      <div className="stats-row" style={{ marginBottom: '28px' }}>
        {[
          { icon: <Briefcase size={20}/>, label: 'Active Jobs', value: '4', trend: '2 new this week', color: 'var(--brand)' },
          { icon: <Users size={20}/>, label: 'Total Applicants', value: '272', trend: '+43 this week', color: '#4F7CFF' },
          { icon: <TrendingUp size={20}/>, label: 'Strong Matches', value: '51', trend: 'AI-selected', color: '#F4A900' },
          { icon: <Calendar size={20}/>, label: 'Interviews Scheduled', value: '6', trend: 'This month', color: '#8B5CF6' },
        ].map((s, i) => (
...
const EmployerDashboard = () => {
  ...
  const renderPanel = () => {
    switch (activePage) {
      case 'dashboard': return <EmployerHome user={user} onNavigate={setActivePage} />;
      ...
      default: return <EmployerHome user={user} onNavigate={setActivePage} />;
    }
  };
